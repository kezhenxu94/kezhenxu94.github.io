...
```vim
:vimgrep "@Column(.*\(COMPONENT_ID\|COMPONENT_IDS\|TIMESTAMP\|START_TIME\|ENTITY_ID\|TAG_KEY\|LATENCY\|START_TIME_SECOND\|START_TIME_NANOS\|VALUE\|TIME_BUCKET\|PERCENTAGE\|OPERATION_TIME\|CREATE_TIME\|OPERATION_TIME\|DUMP_TIME\|SEQUENCE\|TRACE_ID\|TIMESTAMP_MILLIS\)" **/*.java
```
...
It shows there are 218 files that need to be modified. You can also use the following command to open the quickfix window to see the list of files:
